...
def ensure_supported_python() -> None:
    ...
    if version_tuple not in SUPPORTED_PYTHON:
        raise RuntimeError(
            "Unsupported Python version for this training stack: "
            f"{sys.version.split()[0]}. Use Python 3.10, 3.11, or 3.12 (recommended: 3.11)."
        )


def import_unsloth_model():
    try:
        from unsloth import FastLanguageModel
    except Exception as exc:  # noqa: BLE001
        raise RuntimeError(
            "Failed to import Unsloth. Ensure you are using a virtualenv with Python 3.10-3.12 "
...
def import_runtime_dependencies() -> tuple[Any, Any, Any, Any]:
    try:
        ...
        raise RuntimeError(
            "Failed to import required training dependencies. Activate your project virtualenv and run "
            "`pip install -r requirements.txt`."
        ) from exc
    return load_dataset, LoraConfig, TrainingArguments, SFTTrainer
...
def load_config(path: Path) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(
            "Missing PyYAML dependency. Activate your project virtualenv and run "
            "`pip install -r requirements.txt`."
        ) from exc
...
def main() -> None:
    ...
    FastLanguageModel = import_unsloth_model()
    load_dataset, LoraConfig, TrainingArguments, SFTTrainer = import_runtime_dependencies()
